...
# This script is used to plot annual historical-future signals in water temperature 
# for all rcps (separate plot per rcp)
    ...
    for mod in models:
        for rcp in rcps:
            for file in [file for file in sorted(os.listdir(inDIR))\
                         if mod+'_'+rcp+'_'+var+'_'+tscale+'_'+prod in file]:
                    files.append(file)
    ...
    for rcp in rcps:
        ...
        for mod in available_models:
            for file in files:
                count = 0
                if mod != 'lakemod':
                    if rcp in file and mod in file in file:
                        arrays[rcp][mod] = reader(file)
                        count += 1
                        if count == 1:
                            if mod == 'clm45': # grabbing new coord dims for LAKE data-arrays
                                clm_lat = arrays[rcp][mod].lat
                                clm_lon = arrays[rcp][mod].lon
                elif mod == 'lakemod':
                    if rcp in file and mod in file:
                        arrays[rcp][mod] = LAKEreader(file,clm_lon,clm_lat)
    
    # add key for multi-model mean
    for rcp in rcps:    
        temp_list = []
        for mod in available_models:
            temp_list.append(arrays[rcp][mod])
        arrays[rcp]['mmm'] = ensembler(temp_list)
    
    multimodel = {}
    for rcp in rcps:    
        multimodel[rcp] = [arrays[rcp]['mmm'],\
                           arrays[rcp]['clm45'],\
                           arrays[rcp]['simstrat-uog'],\
                           arrays[rcp]['albm'],\
                           arrays[rcp]['vic-lake']]    
    
    lat = arrays['rcp26']['clm45'].lat.values
    lon = arrays['rcp26']['clm45'].lon.values
