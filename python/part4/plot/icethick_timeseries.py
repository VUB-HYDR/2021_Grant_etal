...
#This script loops through annual fieldmeans of each lake model's icethick data for
...
# processing function
def it_proc(inDIR,flag_main_SI):
        ...
        for mod in available_models:
            ...
            for gcm in forcings:
                ...
                if len(temp_list) > 1:
                    ...
                    pi_refs[mod][gcm] = temp_list[0].mean(dim='time').values
    ...
    if flag_main_SI == 1:
        ...
        return anoms,filesets,datasets
    
# plotting function for subplots
def it_plot(datasets,outDIR,var,prod,flag_svplt,dpi):
    
    # list of model titles
    model_titles = ['CLM4.5', 'ALBM', 'SIMSTRAT-UoG','LAKE']
    ...
    # figsize = (x,y), 
    # eg  2 row 1 col -> (5,5); 4 row 1 col -> (10,5)
    # add 1 to y for legend
    x = 5
    y = 9
    ...
    if flag_svplt == 0:
        ...
        f.savefig(outDIR+'/si_f24.png',bbox_inches='tight',dpi=dpi)